# Modified from https://github.com/siyka-au/pygeocom/tree/master
from abc import ABC
...
@dataclass
class FullMeasurementResponse(AngleResponse):
    distance: float
...
class Station:
    ...
    def plus(self, other_station: "Station") -> "Station":
        return Station(
            e0=self.e0 + other_station.e0,
            n0=self.n0 + other_station.n0,
            h0=self.h0 + other_station.h0,
            hi=self.hi + other_station.hi,
        )

    def minus(self, other_station: "Station") -> "Station":
        return Station(
            e0=self.e0 - other_station.e0,
            n0=self.n0 - other_station.n0,
            h0=self.h0 - other_station.h0,
...
class PrismType(Enum):
    ...
    LEICA_MPR122 = 12  # Prism type: Leica MPR122 for machine guidance
...
class MeasurementProgram(Enum):
    ...
    CONT_REF_SYNCHRO = 10  # tracking IR distance synchro
    SINGLE_REF_PRECISE = 11  # single IR distance precise
...
class TMCMeasurementMode(Enum):
    ...
    RED_LASER_TRACKING = 10  # Red laser tracking (reflectorless)
...
def default_return_code_handler(return_code: int):
    if return_code != ReturnCode.GRC_OK:
        logger.error(ReturnCodeException(return_code))
...
class PyGeoCom:
    ...
    def _request(
        ...
        return (geocom_return_code, rpc_return_code, resp_time, resp_len) + tuple(p)

    def get_instrument_number(self) -> int:
        _, _, _, _, instrument_number = self._request(5003)
        return int(instrument_number)

    def get_instrument_name(self) -> str:
        _, _, _, _, instrument_name = self._request(5004)
        return decode_string(instrument_name)

    def get_config(self) -> Tuple[int, int, int]:
        _, _, _, _, reserved, auto_power, time_out = self._request(14001)
        return int(reserved), int(auto_power), int(time_out)

    def set_config(self, reserved: int, auto_power: int, time_out: int) -> None:
        self._request(14002, byte(reserved), byte(auto_power), byte(time_out))

    def get_device_config(self) -> DeviceType:
        _, _, _, _, device_class, device_type = self._request(5035)
        return DeviceClass(int(device_class)), DeviceType(int(device_type))

    def get_date_time(self) -> datetime:
        _, _, _, _, year, month, day, hour, minute, second = self._request(5008)
        year = int(year)
        month = byte(month)
        day = byte(day)
        hour = byte(hour)
    ...
    def get_software_version(self) -> Tuple[int, int, int]:
        _, _, _, _, release, version, subversion = self._request(5034)
        return int(release), int(version), int(subversion)

    def check_power(self) -> Tuple[int, PowerPath, PowerPath]:
        _, _, _, _, capacity, active_power, power_suggest = self._request(5039)
        return (
            int(capacity),
    ...
    def get_double_precision_setting(self) -> int:
        _, _, _, _, number_of_digits = self._request(108)
    ...
    def get_egl_intensity(self) -> EGLIntensity:
        _, _, _, _, intensity = self._request(1058)
    ...
    def get_motor_lock_status(self) -> LockInStatus:
        _, _, _, _, motor_lock_status = self._request(6021)
    ...
    def get_target_type(self) -> TargetType:
        _, _, _, _, target_type = self._request(17022)
    ...
    def get_prism_type(self) -> PrismType:
        _, _, _, _, prism_type = self._request(17009)
    ...
    def get_prism_definition(self, prism_type: PrismType) -> Tuple[str, float, ReflectorType]:
        _, _, _, _, name, correction, reflector_type = self._request(17023, (prism_type.value,))
    ...
    def get_measurement_program(self) -> MeasurementProgram:
        _, _, _, _, measurement_program = self._request(17018)
    ...
    def measure_distance_and_angles(
        ...
        _, _, _, _, horizontal, vertical, distance, measurement_mode = self._request(17017, (measurement_mode.value,))
    ...
    def set_atr_precise(self):
        self._request(17040, (0,))
    ...
    def get_server_software_version(self) -> Tuple[int, int, int]:
        _, _, _, _, release, version, subversion = self._request(110)
        return int(release), int(version), int(subversion)
    ...
    def get_user_atr_state(self) -> OnOff:
        _, _, _, _, atr_state = self._request(18006)
    ...
    def get_user_lock_state(self) -> OnOff:
        _, _, _, _, lock_state = self._request(18008)
    ...
    def get_rcs_search_switch(self) -> OnOff:
        ...
        :rtype: OnOff
        """
        _, _, _, _, search_switch = self._request(18010)
    ...
    def get_tolerance(self) -> Tuple[float, float]:
        _, _, _, _, horizontal_tolerance, vertical_tolerance = self._request(9008)
    ...
    def get_positioning_timeout(self) -> Tuple[float, float]:
        _, _, _, _, horizontal_timeout, vertical_timeout = self._request(9012)
    ...
    def get_fine_adjust_mode(self) -> FineAdjustPositionMode:
        _, _, _, _, fine_adjust_mode = self._request(9030)
    ...
    def get_search_spiral(self) -> Tuple[float, float]:
        _, _, _, _, horizontal_range, vertical_range = self._request(9040)
    ...
    def get_signal(self):
        _, _, _, _, signal_intensity, time = self._request(2022)
        return float(signal_intensity), int(time)

    def get_face(self) -> int:
        _, _, _, _, face = self._request(2026)
        return int(face)

    def get_coordinate(self, inclination_mode: TMCInclinationMode, wait_time: int = 1000) -> CoordinateResponse:
        (
            geocom_return_code,
            rpc_return_code,
            resp_time,
        ...
        return CoordinateResponse(
            geocom_return_code=int(geocom_return_code),
            rpc_return_code=int(rpc_return_code.value),
            resp_time=resp_time,
            resp_len=resp_len,
            time=float(measure_time_cont),
            x=float(east_cont),
            y=float(north_cont),
            z=float(height_cont),
        )
    ...
    def get_full_measurement(
        ...
        return FullMeasurementResponse(
            time=float(measure_time),
            geocom_return_code=int(geocom_return_code),
            rpc_return_code=int(rpc_return_code.value),
            resp_time=resp_time,
            resp_len=resp_len,
            h_angle=float(horizontal),
            v_angle=float(vertical),
            distance=float(slope_distance),
        )

    def get_simple_measurement(
        self, inclination_mode: TMCInclinationMode, wait_time: int = 1000
    ) -> Tuple[Angles, float]:
        _, _, _, _, horizontal, vertical, slope_distance = self._request(
            2108,
            (
                wait_time,
    ...
    def get_angles_simple(self, inclination_mode: TMCInclinationMode) -> Angles:
        _, _, _, _, horizontal, vertical = self._request(2107, (inclination_mode.value,))
        return Angles(float(horizontal), float(vertical))

    def get_angles_complete(self, inclination_mode: TMCInclinationMode) -> AngleResponse:
        (
            geocom_return_code,
            rpc_return_code,
            resp_time,
    ...
    def get_station(self, station: Station = Station()) -> Station:
        _, _, _, _, e0, n0, h0, hi = self._request(2009, tuple(station.__dict__.values()))
        return Station(e0=float(e0), n0=float(n0), h0=float(h0), hi=float(hi))

    def set_station(self, station: Station = Station()) -> Tuple[float]:
        self._request(2010, tuple(station.__dict__.values()))

    def set_orientation(self, orientation: float) -> Tuple[float]:
        self._request(2113, (orientation,))
    ...
    def prepare_static_measurement(self, tracking_settings: TrackingSettings):
        self.set_prism_type(PrismType(tracking_settings.prism_type))
        self.user_atr_state_on()
        self.set_fine_adjust_mode(FineAdjustPositionMode(tracking_settings.fine_adjust_position_mode))
        self.fine_adjust(
            tracking_settings.fine_adjust_horizontal_search_range,
            tracking_settings.fine_adjust_vertical_search_range,
        )
        self.set_edm_mode(EDMMeasurementMode.SINGLE_STANDARD)
        self.do_measure(TMCMeasurementMode.DEFAULT_DISTANCE, TMCInclinationMode.AUTOMATIC)
    ...
    def start_tracking(
        ...
        self.set_search_area(
            tracking_settings.power_search_area_dcenterhz,
            tracking_settings.power_search_area_dcenterv,
            tracking_settings.power_search_area_drangehz,
            tracking_settings.power_search_area_drangev,
            tracking_settings.power_search_area_enabled,
        )
        self.ps_set_range(
            tracking_settings.power_search_min_range,
            tracking_settings.power_search_max_range,
        )
        self.ps_enable_range(BOOLE.TRUE)

        if tracking_settings.power_search:
            self.ps_search_window()
        else:
            self.fine_adjust(
                tracking_settings.fine_adjust_horizontal_search_range,
                tracking_settings.fine_adjust_vertical_search_range,
        ...
        self.user_lock_state_on()
        self.lock_in()

        self.set_edm_mode(EDMMeasurementMode(tracking_settings.edm_measurement_mode))
        self.do_measure(
    ...
    def stop_tracking(self):
        self.user_lock_state_off()
        self.set_edm_mode(EDMMeasurementMode.SINGLE_STANDARD)
        self.do_measure(TMCMeasurementMode.STOP_AND_CLEAR, TMCInclinationMode.AUTOMATIC)
        self.set_egl_intensity(OnOff.OFF)
